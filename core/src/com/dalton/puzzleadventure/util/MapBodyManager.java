package com.dalton.puzzleadventure.util;
...
public class MapBodyManager {
	...
	public void createPhysics(Map map, String layerName) {
		...
		while(objectIt.hasNext()) {
            ...
            //Gdx.app.log("MapBodyManagerDebug", "Created new body at: " + body.getPosition()); //TO DO Remove this

            bodies.add(body);
			
			fixtureDef.shape = null; //TO DO Change this back
			shape.dispose();
		}
	}
     ...
     * @param layerName name of the layer that contains the shapes.
     */
    public void createZones(Map map, String layerName, GameWorld gameWorld) {
        MapLayer layer = map.getLayers().get(layerName);
        ...
        while(objectIt.hasNext()) {
            ...
            //Find which type of zone this is
            String zoneName = object.getProperties().get("type", String.class);
            Class<? extends Zone> zoneClass = GameScreen.getZoneFromName(zoneName);

            //Gdx.app.log("MapBodyManager", "Zone Name: " + zoneName + " Class: " + zoneClass);

            if (zoneClass != null)
            {
                Gdx.app.log("MapBodyManager", "Attempting to spawn " + zoneClass.getSimpleName());

                try
                {
                    Constructor<? extends Zone> constructor = zoneClass.getConstructor(Body.class, GameWorld.class, MapObject.class);
                    Zone zone = constructor.newInstance(body, gameWorld, object);
                }
                catch (Exception e)
                {
                    e.printStackTrace();
            ...
            /*Gdx.app.log("MapBodyManager", "Damage Zone Properties: ");
            Iterator<String> iterator = object.getProperties().getKeys();
            while (iterator.hasNext())
            {
                String next = iterator.next();
                Gdx.app.log("MapBodyManager", "Key: " + next + " Value: " + object.getProperties().get(next));
            }* /
            ...
            catch (NumberFormatException e)
            {
                ...
                Gdx.app.log("MapBodyManager", e.getMessage() + "\nInvalid damage zone, damageDelay not a parseable integer, ignoring");
            }

            if (damageDelay > 0) //If the damage zone's properties are valid
            {
                DamageZone damageZone = new DamageZone(body, object);
            }*/
	...
	private Shape getPolygon(PolygonMapObject polygonObject) {
        ...
        //Gdx.app.log("MapBodyManagerDebug", "Shape vertices: " + Arrays.toString(worldVertices));
